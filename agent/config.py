...
def _require(key: str) -> str:
    ...
    if not value:
        raise EnvironmentError(
            f"Required environment variable '{key}' is missing or empty.\n"
            f"Add it to your .env file (local) or platform environment (Railway/Render).\n"
...
def _load_settings() -> Settings:
    ...
    Loads .env file if present (local development).
    On Railway/Docker, environment variables are injected directly — no .env needed.

    Returns:
        A fully populated, immutable Settings instance.

    Raises:
        EnvironmentError: If any required variable is absent or malformed.
    """
    env_path = Path(".env")

    # Load .env if it exists (local dev only).
    # On Railway/Render/Docker, env vars are injected by the platform — skip silently.
    if env_path.exists():
        load_dotenv(dotenv_path=env_path)
        logger.debug("Loaded .env from %s", env_path.resolve())
    else:
        logger.debug("No .env file found — expecting environment variables from platform.")
